from control_traffic_lights import calculateReward, generateRandomRoutes2, getSimStatebyImage, getSimStatebyNumbers, setSig1, setSig2
import traci
import os
import sumolib
import numpy as np 
...
class SumoEnv:
    ...
    def reset(self):
        if self.sumo_process is not None:
            generateRandomRoutes2() 
            self.prev_action = -1
            traci.load(["-c", self.sumo_config, "--start", "--step-length", "1", "--delay", "20"])
        else:
            self.start()
    ...
    def is_done(self):
        ...
        if step <= 2000 and not stop_simulation:  # 2000
            return False
        else:
            return True
