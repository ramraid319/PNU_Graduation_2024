...
class QNet(nn.Module):
    def __init__(self, action_size):
        ...
        self.conv1 = nn.Conv2d(1, 32, kernel_size=8, stride=4)  # Input channels: 1 (grayscale), Output: 32
        self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2)  # Output: 64
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)  # Output: 64

        self.fc1 = nn.Linear(64 * 6 * 6, 512)  # Adjust size after conv layers
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.size(0), -1)  # Flatten the convolution output
        x = F.relu(self.fc1(x))
